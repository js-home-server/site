...
<style>
	...
	/* The dark box the live grid stands in, tagged the way the crypto study's
	   tape is: a window into a real feed, not a panel of this card. */
	.proof {
		/* The height the grid inside is laid out to. Its own, rather than the
		   one .visual hands the other cards' boxes: nothing else in this study
		   is the same shape. */
		--visual-h: 32rem;
	...
	/* /server's own overview grid (routes/server/+page.svelte), copied and
	   ...
	   frame the individual .obox tiles read their dark tokens from, the way
	   .projects itself shadows them light for everything else in it, just run
	   the other way. Its own background would have been the exact tone every
	   .obox already paints itself, which is what read as one big panel
	   wrapping the tiles rather than the tiles standing on their own. */
	.mini-dashboard {
