<script>
	/* The one place the site's arrow convention is decided: site for anywhere
	   ...
	   gone to. Everything that used to pick its own glyph (and, in two places,
	   picked the wrong one for a download) points here instead. */
	const ARROWS = { site: '→', external: '↗', download: '↓' };
...
<a
	{href}
	class="action-link {variant} {direction} {extraClass}"
...
<style>
	...
	/* → alone has no vertical stroke of its own to sit level on the baseline
	   with — at this size that reads as sunk below the text beside it, so it is
	   the one glyph centred on the line rather than set on its baseline. */
