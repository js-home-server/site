<script>
	// Generated by ../asciiArt: `make site` there rewrites AsciiShip.svelte.
	...
	import { bucket, summarise } from '$lib/stats.js';
	import { volume } from '$lib/storage.js';

	/* Wireframe. CPU and storage are wired up; everything else is a placeholder
	   holding the shape of what goes there, and no ascii art is generated yet.
	   One entry per section of the page and per stop on the rail: a section is a
	   box on the page, its own heading, and its own place in the list. */
	const SECTIONS = [
		{ id: 'cpu', label: 'CPU' },
	...
	/* The headline reading, to the precision it is worth quoting at. */
	const reading = (key, unit, dp = 1) =>
		snapshot ? `${snapshot[key].toFixed(dp)}${unit}` : '—';
		...
		DISKS.flatMap(({ id, label, tone }) =>
			['Read', 'Write'].map((direction) => {
				...
				return {
					...
					group: label,
					tone,
					now: points?.length ? rate(points.at(-1)[1]) : '—',
	...
	let cores = $derived(
		Object.entries(series?.cpuPerCorePercent ?? {})
			...
			.map(([id, points]) => ({
				id: `C${id}`,
				now: points.length ? pct(points.at(-1)[1]) : '—',
				cells: bucket(points, HEAT_COLUMNS)
			}))
	);
</script>
...
{#snippet stub(label, note, lines = 1)}
	<Panel {label}>
...
{#snippet cpu()}
	<div class="stack">
		...
		<MetricRow
			label="Pressure"
			value={reading('cpuPressurePercent', '%')}
			tone="var(--coral)"
		...
		<MetricRow
			label="Temperature"
			value={reading('cpuTemperatureC', '°C', 0)}
			tone="var(--amber)"
...
{#snippet containers()}
	...
	<div class="ship" aria-hidden="true"><AsciiShip /></div>
	<div class="grid">
...
<div class="server">
	...
	<div class="body">
		<header class="masthead">
			<h1>Server</h1>
			<p class="state">Placeholder — no data wired up</p>
			<p class="meta">host · uptime · kernel</p>
			<!-- One banner for the page, under its title, rather than one at the head
			     of every section. -->
			<div class="banner"><Placeholder note="ascii banner" lines={8} /></div>
		</header>
...
<style>
	...
	/* The art is a fixed grid — 166 characters wide — so its only size is how big
	   one character cell is, and the cell is picked so the ship spans the box it
	   sits in. JetBrains Mono advances 0.6021em, so the grid is 166 x 0.6021 =
	   99.95 cells across; dividing the box by that puts a cell at about 9.5px in
	   this column, which is the size the bull's own digits are set at on a laptop
	   screen. Narrower boxes shrink both together. */
	.ship {
