<script>
	...
	let latencyP95 = $derived(ms(percentile(values(series?.latencyMs), 0.95)));
	...
	<div class="box fill" style={gridArea({ col: 3, row: 1, w: 2, h: 2 })}>
		<Panel label="HTTP Probe Latency">
			<p class="latency-reading">
				<strong>{ms(snapshot?.latencyMs)}</strong><span>P95 {latencyP95}</span>
			</p>
			<Trace
				lines={[{ id: 'latency', points: series?.latencyMs, tone: 'var(--azure)' }]}
				format={ms}
			/>
		</Panel>
	</div>
